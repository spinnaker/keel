...
internal class CanaryConstraintEvaluatorTests : JUnit5Minutests {
  ...
  data class Fixture(
    ...
    val artifact: ArtifactSpec =
      DebianArtifactSpec(name = "fnord", vmOptions = VirtualMachineOptions(baseOs = "bionic", regions = setOf("us-west-2")), reference = "fnord"),
    ...
    context("promotion is gated on launching a canary and its status") {
      context("no canaries were launched yet") {
        before {
          ...
          every {
            repository.getConstraintState(deliveryConfig.name, targetEnvironment.name, version, constraint.type, artifact.reference)
          } answers {
