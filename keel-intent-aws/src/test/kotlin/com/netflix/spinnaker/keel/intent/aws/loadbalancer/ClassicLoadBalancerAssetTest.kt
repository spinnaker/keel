...
import com.netflix.spinnaker.keel.intent.LoadBalancerAsset
...
object ClassicLoadBalancerAssetTest {
  ...
  fun `can deserialize from expected JSON format`() {
    mapper.readValue<LoadBalancerAsset>(json).apply {
  ...
  fun `availability zones defaults to automatic`() {
    mapper.readValue<Map<String, Any>>(json)
      ...
      .let {
        mapper.convertValue<LoadBalancerAsset>(it).apply {
