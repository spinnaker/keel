...
class UnhealthyVetoTests : JUnit5Minutests {
  ...
  fun tests() = rootContext<Fixture> {
    ...
    context("resource hasn't been marked unheathy") {
      before {
    ...
    context("Resource was marked unhealthy") {
      before {
      ...
      context("resource was marked unhealthy 1 minute ago") {
        before {
          every { unhealthyRepository.getUnhealthyTime(r.id) } returns clock.instant().minusSeconds(60)
          check()
        }
      ...
      context("resource was marked unhealthy 4 minute ago") {
        before {
          every { unhealthyRepository.getUnhealthyTime(r.id) } returns clock.instant().minusSeconds(60 * 4)
          check()
        }
      ...
      context("resource was marked unhealthy 6 minutes ago") {
        before {
          every { unhealthyRepository.getUnhealthyTime(r.id) } returns clock.instant().minusSeconds(60 * 6)
          check()
        }
