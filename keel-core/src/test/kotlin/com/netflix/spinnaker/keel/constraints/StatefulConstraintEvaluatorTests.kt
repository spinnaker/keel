...
internal class StatefulConstraintEvaluatorTests : JUnit5Minutests {
  ...
  fun tests() = rootContext<Fixture> {
    ...
    before {
      ...
      every {
        repository.getConstraintState("test", "test", "v1.0.0", "fake", artifact.type)
      } returns null

      every {
        repository.getConstraintState("test", "test", "v1.0.1", "fake", artifact.type)
    ...
    test("canPromote does NOT generate an event after the first state change") {
      ...
      verify(exactly = 2) {
        repository.getConstraintState("test", "test", any(), "fake", artifact.type)
      }
