...
class ComparatorSortingTests : JUnit5Minutests {
  ...
  fun tests() = rootContext<Unit> {
    ...
    context("increasing version") {
      test("descending by default") {
        expectThat(incrTags.sortedWith(DockerVersionSortingStrategy(INCREASING_TAG).comparator).first()).isEqualTo("3")
      }
    }

    context("semver version") {
      test("descending by default") {
        expectThat(semverTags.sortedWith(DockerVersionSortingStrategy(SEMVER_TAG).comparator).first()).isEqualTo("v0.10.3")
      }
    }

    context("branchjobcommit version") {
      test("descending by default") {
        expectThat(branchJobCommitTags.sortedWith(DockerVersionSortingStrategy(BRANCH_JOB_COMMIT_BY_JOB).comparator).first()).isEqualTo("master-h3.blah")
      }
    }

    context("semver job commit version") {
      test("descending by default, by job") {
        expectThat(semverJobCommitTags.sortedWith(DockerVersionSortingStrategy(SEMVER_JOB_COMMIT_BY_JOB).comparator).first()).isEqualTo("v1.12.3-rc.1-h1192.f876e5a")
      }

      test("descending by default, by version") {
        expectThat(semverJobCommitTags.sortedWith(DockerVersionSortingStrategy(SEMVER_JOB_COMMIT_BY_SEMVER).comparator).first()).isEqualTo("v1.12.3-rc.1-h1192.f876e5a")
      }
    }
