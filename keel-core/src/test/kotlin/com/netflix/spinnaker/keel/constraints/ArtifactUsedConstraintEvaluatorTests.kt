...
internal class ArtifactUsedConstraintEvaluatorTests : JUnit5Minutests {
  object Fixture {
    ...
    val debian = DebianArtifactSpec("fnord", reference = "debFnord", deliveryConfigName = manifestName, vmOptions = VirtualMachineOptions(baseOs = "bionic", regions = setOf("us-east-1", "us-west-2")))
    val docker = DockerArtifactSpec("fnord", reference = "dockerfnord", deliveryConfigName = manifestName)
    val anotherDebian = DebianArtifactSpec("fnord", reference = "fnord2.0", statuses = setOf(SNAPSHOT), deliveryConfigName = manifestName, vmOptions = VirtualMachineOptions(baseOs = "bionic", regions = setOf("us-east-1", "us-west-2")))
