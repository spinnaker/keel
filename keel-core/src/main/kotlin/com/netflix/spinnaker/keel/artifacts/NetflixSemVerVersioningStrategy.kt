...
object NetflixSemVerVersioningStrategy : VersioningStrategy {
  ...
  private val NPM_VERSION_REGEX = Regex("(\\d+\\.\\d+\\.\\d+(-rc(\\.\\d+)?)?)(-[h]?\\w+)?(-\\w+)?")
  ...
  fun getVersionDisplayName(artifact: PublishedArtifact): String {
    return NPM_VERSION_REGEX.find(artifact.version)?.groups?.get(1)?.value
      ?: artifact.version
  }
  ...
  fun getBuildNumber(artifact: PublishedArtifact): Int? {
    return NPM_VERSION_REGEX.find(artifact.version)?.groups?.get(4)?.value
      ?.let {
  ...
  fun getCommitHash(artifact: PublishedArtifact): String? {
    return NPM_VERSION_REGEX.find(artifact.version)?.groups?.get(5)?.value
      ?.let { it.replace("-", "") }
  }
