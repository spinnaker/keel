  ...
  fun exportFromPipelines(application: String, serviceAccount: String): SubmittedDeliveryConfig {
    val pipelines = runBlocking {
      ...
      .filter { pipeline ->
        ...
        // TODO: filter pipelines that have zero executions
      }
    ...
    val pipelinesToEnvironments = pipelinesToStagesToArtifacts.mapNotNull { (pipeline, stageToArtifact) ->
      ...
      val environment = SubmittedEnvironment(
        name = deploy.clusters.first().name,
        resources = setOf(
          SubmittedResource(
            kind = kind,
      ...
      return@mapNotNull pipeline to environment
    }.toMap()

    val environments = pipelinesToEnvironments.map { (pipeline, environment) ->
      ...
      environment.copy(constraints = environment.constraints + constraints)
    }
      .also { environments ->
